...
const EventCard = ({ eventId }: { eventId: Id<"events"> }) => {
    ...
    if (userTicket) {
      return (
        <div className="mt-4 flex items-center justify-between p-3 bg-green-50 rounded-lg border border-green-100">
          ...
          <button
            className="bg-green-600 hover:bg-green-700 text-white px-3 py-1.5 rounded-full font-medium shadow-sm transition-colors duration-200 flex items-center gap-1"
  ...
  return (
    <div
      ...
      className={`bg-white rounded-xl shadown-sm hover:shadow-lg transition-all duration-300 border border-gray-100 cursor-pointer overflow-hidden relative ${isPastEvent ? "opacity-75 hover:opacity-75" : ""}`}
    >
      {/* Event Image */}
